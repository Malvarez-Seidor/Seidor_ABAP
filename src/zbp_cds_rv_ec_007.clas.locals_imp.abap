CLASS lhc_Constants DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    DATA update_allowed TYPE abap_bool.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR Constants RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR Constants RESULT result.
...
CLASS lhc_Constants IMPLEMENTATION.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_007  IN LOCAL MODE
       ENTITY Constants
       FIELDS ( Companycode Api Fieldname Sequence Sign Options Low High )
       WITH CORRESPONDING #( keys )
       RESULT DATA(Constants).

     CHECK Constants is not initial.

     update_requested = COND #( WHEN requested_authorizations-%update = if_abap_behv=>mk-on
                             THEN abap_true ELSE abap_false ).
     result = VALUE #( FOR Constant IN Constants ( %tky = Constant-%tky ) ).
