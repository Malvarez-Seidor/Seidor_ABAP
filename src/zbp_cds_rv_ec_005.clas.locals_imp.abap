CLASS lhc_TypeCondition DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    DATA update_allowed TYPE abap_bool.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR TypeCondition RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR TypeCondition RESULT result.
...
CLASS lhc_TypeCondition IMPLEMENTATION.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_005  IN LOCAL MODE
       ENTITY TypeCondition
       FIELDS ( Companycode Typecondition conditionapplication Ccondition )
     ...
     result = VALUE #( FOR Condition IN Conditions ( %tky = Condition-%tky ) ).
