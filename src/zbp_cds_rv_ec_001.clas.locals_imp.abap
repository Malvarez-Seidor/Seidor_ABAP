CLASS lhc_ElectronicDocuments DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS createses FOR MODIFY
      IMPORTING keys FOR ACTION electronicdocuments~createses.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_001  IN LOCAL MODE
       ENTITY ElectronicDocuments
       FIELDS ( Companycode Documentsri Documenttype Export Reason Refunds Sequence )
       WITH CORRESPONDING #( keys )
       RESULT DATA(Documents).

     CHECK Documents is not initial.

     update_requested = COND #( WHEN requested_authorizations-%update = if_abap_behv=>mk-on
                             THEN abap_true ELSE abap_false ).
     result = VALUE #( FOR Document IN Documents ( %tky = Document-%tky ) ).
