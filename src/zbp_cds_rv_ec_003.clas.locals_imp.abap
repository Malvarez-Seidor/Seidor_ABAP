CLASS lhc_TypesTaxes DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    DATA update_allowed TYPE abap_bool.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR typestaxes RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR TypesTaxes RESULT result.
...
CLASS lhc_TypesTaxes IMPLEMENTATION.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_003  IN LOCAL MODE
       ENTITY TypesTaxes
       FIELDS ( Companycode Taxcode Taxsupportid Taxratepercent Taxsidrate Tax0 Tax Notax Exempttax )
       WITH CORRESPONDING #( keys )
       RESULT DATA(Taxes).

     CHECK Taxes is not initial.

     update_requested = COND #( WHEN requested_authorizations-%update = if_abap_behv=>mk-on
                             THEN abap_true ELSE abap_false ).
     result = VALUE #( FOR Taxe IN Taxes ( %tky = Taxe-%tky ) ).
