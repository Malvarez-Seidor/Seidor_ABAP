CLASS lhc_SriCountries DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    DATA update_allowed TYPE abap_bool.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR SriCountries RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR SriCountries RESULT result.
...
CLASS lhc_SriCountries IMPLEMENTATION.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_009  IN LOCAL MODE
       ENTITY SriCountries
       FIELDS ( Country Countrysri Pais_conv Taxhavencountry )
       WITH CORRESPONDING #( keys )
       RESULT DATA(Countries).

     CHECK Countries is not initial.

     update_requested = COND #( WHEN requested_authorizations-%update = if_abap_behv=>mk-on
                             THEN abap_true ELSE abap_false ).
     result = VALUE #( FOR Country IN Countries ( %tky = Country-%tky ) ).
