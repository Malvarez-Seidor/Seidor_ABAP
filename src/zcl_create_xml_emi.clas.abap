CLASS zcl_create_xml_emi DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS pagosSustento     CHANGING  xml    TYPE string.

    METHODS detail_impuestos  IMPORTING codigoprincipal TYPE zts_det_imp-codigoprincipal
                                        codigoitem      TYPE zts_det_imp-codigoitem
                              CHANGING  xml    TYPE string.
    ...
    METHODS Factura           IMPORTING header    TYPE zts_fac_header
                                        ...
                                        impuesto  TYPE ty_impuesto
                                        pagos     TYPE ty_pagos
                                        detalle   TYPE ty_detalle_f
                                        det_add   TYPE ty_det_add
                                        det_imp   TYPE ty_det_imp
                                        reembolso TYPE ty_reembolso
                                        reem_imp  TYPE ty_reem_imp
                                        head_add  TYPE ty_head_add
    ...
    METHODS notaCredito_detail   CHANGING  xml   TYPE string.

    METHODS notaDedito IMPORTING header    TYPE zts_nd_header
                                 inf_tribu TYPE zts_inf_tribu
                                 impuesto  TYPE ty_impuesto
                                 pagos     TYPE ty_pagos
                                 motivos   TYPE ty_motivos
                                 head_add  TYPE ty_head_add
                        EXPORTING xml      TYPE string.
    ...
    METHODS retencion IMPORTING  header    TYPE zts_rete_header
                                 inf_tribu TYPE zts_inf_tribu
                                 sustento  TYPE ty_sustento
                                 impuesto  TYPE ty_imp_sust
                                 retencion TYPE ty_retencion
                                 reembolso TYPE ty_reembolso
                                 reem_imp  TYPE ty_reem_imp
                                 pagos     TYPE ty_pagos
                                 head_add  TYPE ty_head_add
...
CLASS zcl_create_xml_emi IMPLEMENTATION.
  ...
  METHOD detail_impuestos.
    ...
    LOOP AT me->gt_det_imp INTO gs_det_imp WHERE ( codigoprincipal = codigoprincipal AND codigoitem      = codigoitem ).
    ...
    IF line_exists( me->gt_reem_imp[ secuencialdocreembolso = lv_secuencialdocreembolso ] ).
      ...
      LOOP AT me->gt_reem_imp INTO gs_reem_imp WHERE secuencialdocreembolso = lv_secuencialdocreembolso.
        ...
        IF gs_reem_imp-impuestoreembolso IS NOT INITIAL.
          CONDENSE gs_reem_imp-impuestoreembolso NO-GAPS.
  ...
  METHOD factura_detail.
    ...
    IF me->gt_detalle_f[] IS NOT INITIAL.
      ...
      LOOP AT me->gt_detalle_f INTO gs_detalle_f.
        ...
        me->detail_impuestos( EXPORTING codigoprincipal = gs_detalle_f-codigoprincipal
...
*    ENDIF.
    ...
    IF me->gs_header_f-totalSinImpuestos IS NOT INITIAL.
      CONDENSE me->gs_header_f-totalsinimpuestos NO-GAPS.
...
*    ENDIF.
    ...
    IF me->gs_header_f-totaldescuento IS NOT INITIAL.
      CONDENSE me->gs_header_f-totaldescuento NO-GAPS.
    ...
    IF me->gs_header_f-totalcomprobantesreembolso IS NOT INITIAL.
      CONDENSE me->gs_header_f-totalcomprobantesreembolso NO-GAPS.
    ...
    IF me->gs_header_f-totalbaseimponiblereembolso IS NOT INITIAL.
      CONDENSE me->gs_header_f-totalbaseimponiblereembolso NO-GAPS.
    ...
    IF me->gs_header_f-totalimpuestoreembolso IS NOT INITIAL.
      CONDENSE me->gs_header_f-totalimpuestoreembolso NO-GAPS.
    ...
    IF me->gs_header_f-propina IS NOT INITIAL.
      CONDENSE me->gs_header_f-propina NO-GAPS.
...
*        CONDENSE me->gs_header_f-fleteinternacional NO-GAPS.
...
*        CONDENSE me->gs_header_f-segurointernacional NO-GAPS.
...
*        CONDENSE me->gs_header_f-gastosaduaneros NO-GAPS.
...
*        CONDENSE me->gs_header_f-gastostransporteotros NO-GAPS.
...
*    ENDIF.
    ...
    IF me->gs_header_f-importetotal IS NOT INITIAL.
      CONDENSE me->gs_header_f-importetotal NO-GAPS.
    ...
    IF me->gt_detalle_g[] IS NOT INITIAL.
      ...
      LOOP AT me->gt_detalle_g INTO gs_detalle_g.
        ...
        IF gs_detalle_g-cantidad  IS NOT INITIAL.
          CONDENSE gs_detalle_g-cantidad NO-GAPS.
...
*      ENDIF.
  ...
  METHOD liquidacionHeader.
    ...
    IF me->gs_header_l-totalsinimpuestos IS NOT INITIAL.
      CONDENSE me->gs_header_l-totalsinimpuestos.
    ...
    IF me->gs_header_l-totaldescuento IS NOT INITIAL.
      CONDENSE me->gs_header_l-totaldescuento NO-GAPS.
    ...
    IF me->gs_header_l-coddocreembolso IS NOT INITIAL.
      ...
      IF me->gs_header_l-totalcomprobantesreembolso IS NOT INITIAL.
        CONDENSE me->gs_header_l-totalcomprobantesreembolso NO-GAPS.
      ...
      IF me->gs_header_l-totalbaseimponiblereembolso IS NOT INITIAL.
        CONDENSE me->gs_header_l-totalbaseimponiblereembolso NO-GAPS.
      ...
      IF me->gs_header_l-totalimpuestoreembolso IS NOT INITIAL.
        CONDENSE me->gs_header_l-totalimpuestoreembolso NO-GAPS.
    ...
    IF me->gs_header_l-importetotal IS NOT INITIAL.
      CONDENSE me->gs_header_l-importetotal NO-GAPS.
  ...
  METHOD notacredito.
    ...
    IF me->gs_header_c IS NOT INITIAL.
      ...
      IF me->gt_detalle_f[] IS NOT INITIAL."Detalles de Facturas
        ...
        me->notaCredito_detail( CHANGING xml = xml ).
  ...
  METHOD notacreditoheader.
    ...
    CONCATENATE xml '<infoNotaCredito>' INTO xml.
    ...
    IF me->gs_header_c-totalsinimpuestos IS NOT INITIAL.
      CONDENSE me->gs_header_c-totalsinimpuestos NO-GAPS.
    ...
    IF me->gs_header_c-valor_mod IS NOT INITIAL.
      CONDENSE me->gs_header_c-valor_mod NO-GAPS.
  ...
  METHOD notaCredito_detail.
    ...
    IF me->gt_detalle_f[] IS NOT INITIAL.
      ...
      LOOP AT me->gt_detalle_f INTO gs_detalle_f.
        ...
        IF gs_detalle_f-codigoprincipal IS NOT INITIAL.
          CONCATENATE xml '<codigoInterno>' gs_detalle_f-codigoprincipal  '</codigoInterno>' INTO xml.
        ENDIF.
  ...
  METHOD notadeditoheader.
    ...
    CONCATENATE xml '<infoNotaDebito>' INTO xml.
    ...
    IF me->gs_header_d-totalsinimpuestos IS NOT INITIAL.
      CONDENSE me->gs_header_d-totalsinimpuestos NO-GAPS.
    ...
    IF me->gs_header_d-valor IS NOT INITIAL.
      CONDENSE me->gs_header_d-valor NO-GAPS.
    ...
    IF me->gt_motivos[] IS NOT INITIAL.
        ...
        LOOP AT me->gt_motivos INTO gs_motivos.
          ...
          CONDENSE gs_motivos-valor NO-GAPS.
    ...
    CONCATENATE xml '<retenciones>' INTO xml.
     ...
     LOOP AT me->gt_retencion INTO gs_retencion WHERE numdocsustento EQ numdocsustento.
       ...
       IF gs_retencion-baseimponible IS NOT INITIAL.
         CONDENSE gs_retencion-baseimponible NO-GAPS.
       ...
       IF gs_retencion-porcentajeretener IS NOT INITIAL.
         CONDENSE gs_retencion-porcentajeretener NO-GAPS.
       ...
       IF gs_retencion-valorretenido IS NOT INITIAL.
         CONDENSE gs_retencion-valorretenido NO-GAPS.
       ...
       IF gs_retencion-fechapagodiv IS NOT INITIAL.
         ...
         IF gs_retencion-imrentasoc IS NOT INITIAL.
            CONDENSE gs_retencion-imrentasoc NO-GAPS.
  ...
  METHOD retencionSustento.
    ...
    LOOP AT me->gt_sustento INTO gs_sustento.
      ...
      IF gs_sustento-totalcomprobantesreembolso IS NOT INITIAL.
        CONDENSE gs_sustento-totalcomprobantesreembolso NO-GAPS.
      ...
      IF gs_sustento-totalbaseimponiblereembolso IS NOT INITIAL.
        CONDENSE gs_sustento-totalbaseimponiblereembolso NO-GAPS.
      ...
      IF gs_sustento-totalimpuestoreembolso IS NOT INITIAL.
        CONDENSE gs_sustento-totalimpuestoreembolso NO-GAPS.
      ...
      IF gs_sustento-totalsinimpuestos IS NOT INITIAL.
        CONDENSE gs_sustento-totalsinimpuestos NO-GAPS.
      ...
      IF gs_sustento-importetotal IS NOT INITIAL.
        CONDENSE gs_sustento-importetotal NO-GAPS.
      ...
      me->pagosSustento( CHANGING xml = xml ).
  ...
  METHOD totalImpuestoSust.
    ...
    IF me->gt_imp_sust[] IS NOT INITIAL.
      ...
      LOOP AT me->gt_imp_sust INTO gs_imp_sust WHERE numdocsustento EQ numdocsustento.
        ...
        IF gs_imp_sust-codigo IS NOT INITIAL.
          CONCATENATE xml '<codImpuestoDocSustento>' gs_imp_sust-codigo '</codImpuestoDocSustento>' INTO xml.
        ENDIF.

        IF gs_imp_sust-codigoporcentaje  IS NOT INITIAL.
          CONCATENATE xml '<codigoPorcentaje>' gs_imp_sust-codigoporcentaje '</codigoPorcentaje>' INTO xml.
        ENDIF.

        IF gs_imp_sust-baseimponible IS NOT INITIAL.
          CONDENSE gs_imp_sust-baseimponible NO-GAPS.
          CONCATENATE xml '<baseImponible>' gs_imp_sust-baseimponible '</baseImponible>' INTO xml.
        ENDIF.

        IF gs_imp_sust-codigo NE '1' AND gs_imp_sust-tarifa IS NOT INITIAL.
          CONDENSE gs_imp_sust-tarifa NO-GAPS.
          CONCATENATE xml '<tarifa>' gs_imp_sust-tarifa '</tarifa>' INTO xml.
        ENDIF.

        IF gs_imp_sust-valor IS NOT INITIAL.
          CONDENSE gs_imp_sust-valor NO-GAPS.
          CONCATENATE xml '<valorImpuesto>' gs_imp_sust-valor '</valorImpuesto>' INTO xml.
        ENDIF.
    ...
    IF me->gt_impuesto[] IS NOT INITIAL.
      ...
      LOOP AT me->gt_impuesto INTO gs_impuesto.
        ...
        IF gs_impuesto-baseimponible IS NOT INITIAL.
          CONDENSE gs_impuesto-baseimponible NO-GAPS.
...
*      CONCATENATE xml '<comercioExterior>' me->gs_header_f-comercioexterior '</comercioExterior>' INTO xml.
      ...
      IF me->gs_header_f-incotermtotalsin IS NOT INITIAL.
        CONDENSE me->gs_header_f-incotermtotalsin NO-GAPS.
        CONCATENATE xml '<incoTermTotalSinImpuestos>' me->gs_header_f-incotermtotalsin '</incoTermTotalSinImpuestos>' INTO xml.
      ENDIF.
