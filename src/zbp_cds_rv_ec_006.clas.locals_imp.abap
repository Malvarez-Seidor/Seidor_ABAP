CLASS lhc_PaymentConditions DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    DATA update_allowed TYPE abap_bool.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR PaymentConditions RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR PaymentConditions RESULT result.
...
CLASS lhc_PaymentConditions IMPLEMENTATION.
...
*       update result with EDIT Not Allowed
  ...
  METHOD get_instance_authorizations.
    ...
    DATA: update_requested TYPE abap_bool,
     ...
     READ ENTITIES OF zcds_rv_ec_006  IN LOCAL MODE
       ENTITY PaymentConditions
       FIELDS ( Companycode Paymentmethod Paymentsri )
     ...
     result = VALUE #( FOR Payment IN Payments ( %tky = Payment-%tky ) ).
